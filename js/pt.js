function sendit(){
    ...
    const expHpText = /^010\d{3,4}\d{4}$/;  // 앞자리에 010을 꼭 사용하도록 작성
    const expEmailText = /^[A-Za-z0-9\.\-]+@[A-Za-z0-9\.\-]+\.[A-Za-z0-9]+$/;
    const expUserpw = /^[A-Za-z0-9~!@#$%^&*()]+$/;// 비밀번호는 숫자, 대소문자, 특수문자를 꼭 포함하도록 작성
    ...
    // 비밀번호에 숫자, 대소문자, 특수문자가 포함되지 않은 경우
    if(!expUserpw.test(userpw.value)){
        alert('비밀번호에 숫자, 대소문자, 특수문자를 꼭 포함해서 입력하세요.');
        userpw.focus();
        return false;
    }
